...
//	UIImage *image = [theSettings.images valueForKey:[NSString stringWithFormat:@"%i", type]];
    ...
    v.frame = CGRectMake(0, 0, textSize.width + 10 + 30, textSize.height + 10 + 50);
    label.center = CGPointMake(v.frame.size.width / 2, v.frame.size.height - 15 );
	[v addSubview:label];
	[label release];
	
    UIImageView *imageView = [[UIImageView alloc] initWithImage:ImageWithName(@"grieved@2x")];
    imageView.backgroundColor = ColorClear;
    imageView.alpha = 0.8;
    imageView.frame = CGRectMake(0, 0, 45, 45);
    imageView.center = CGPointMake(v.frame.size.width / 2, 30);
	...
	switch (orientation) {
		case UIDeviceOrientationPortrait:
		{
			if (theSettings.gravity == iToastGravityTop) {
				...
				point = CGPointMake(window.frame.size.width / 2, window.frame.size.height/2 + 250);
		...
		case UIDeviceOrientationPortraitUpsideDown:
		{
			...
			if (theSettings.gravity == iToastGravityTop) {
				...
				point = CGPointMake(width/2, height/2 - 50);
			} else {
